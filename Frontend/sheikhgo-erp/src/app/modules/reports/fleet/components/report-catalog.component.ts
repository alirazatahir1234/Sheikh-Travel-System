...
@Component({
  ...
  `,
  styles: [`
    .catalog { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 0.75rem; margin-bottom: 1rem; }
    .catalog-card {
    ...
    @media (max-width: 1300px) { .catalog { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
    @media (max-width: 760px) { .catalog { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
    @media (max-width: 480px) { .catalog { grid-template-columns: 1fr; } }
  `]
})
export class FleetReportCatalogComponent {
  readonly catalog = REPORT_CATALOG;
  readonly selected = input<ReportCatalogId>('trip');
